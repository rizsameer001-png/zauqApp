...
import {
  ...
  ListMusic, Volume2, PlayCircle, Radio, FolderDown,
  MessageSquare, Globe, Zap, Mail, CheckCircle, AlertCircle,
  Newspaper, Edit, Eye, Trash2, Plus, FolderOpen
} from 'lucide-react'
...
const Sidebar = () => {
  ...
  // Admin Menu Items - With Blog CRUD added (using existing icons)
  const adminMenuItems = [
    // Main Admin
    { path: '/admin', icon: Shield, label: 'Admin Dashboard' },
    { path: '/admin/users', icon: Users, label: 'Users' },
    ...
    // ============================================
    { path: '/admin/blog', icon: Newspaper, label: 'Blog CMS', divider: true },
    { path: '/admin/blog/new', icon: Edit, label: 'Write New Post' },
    { path: '/admin/blog/categories', icon: FolderTree, label: 'Blog Categories' },
    { path: '/admin/blog/comments', icon: MessageSquare, label: 'Comments', badge: 'Moderate' },
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-gray-200 fixed h-full z-40">
            ...
            return (
              <React.Fragment key={item.path}>
                ...
                {item.divider && index > 0 && (
                  <li className="pt-4 mt-2 border-t border-gray-100">
                    <p className="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                      {item.label === 'Blog CMS' ? 'Blog Management' : 
                       item.label === 'Audio CMS' ? 'Audio Management' : 
                       item.label === 'Video CMS' ? 'Video Management' :
                       item.label === 'Subscriptions' ? 'Subscription Management' :
                       item.label === 'Site Management' ? 'Site Management' : ''}
      ...
      {isAdmin && (
        <div className="mx-4 mb-4 p-3 bg-gradient-to-r from-primary-50 to-purple-50 rounded-lg border border-primary-100">
          <div className="flex items-center gap-2 mb-2">
            <Newspaper className="h-4 w-4 text-primary-600" />
            <span className="text-xs font-semibold text-primary-700">Blog Stats</span>
          </div>
