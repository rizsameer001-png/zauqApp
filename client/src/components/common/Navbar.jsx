...
//   // 🔴 FIX: Make navLinks dynamic using useMemo to react to language changes
//   const navLinks = useMemo(() => [
//     { path: '/', label: t('nav.home', 'Home'), icon: Compass },
//     { path: '/explore', label: t('nav.explore', 'Explore'), icon: Sparkles },
//     { path: '/poetry', label: t('nav.poetry', 'Poetry'), icon: BookOpen },
//     { path: '/authors', label: t('nav.authors', 'Authors'), icon: User },
//     { path: '/books', label: t('nav.books', 'Books'), icon: Bookmark },
//     { path: '/audio', label: t('nav.audio', 'Audio'), icon: Headphones, dropdown: true },
//     { path: '/videos', label: t('nav.videos', 'Videos'), icon: Video },
//     { path: '/blog', label: t('nav.blog', 'Blog'), icon: Newspaper },
//   ], [t])

//   // 🔴 FIX: Make search categories dynamic with useMemo
//   const searchCategories = useMemo(() => [
//     { id: 'all', label: t('search.all', 'All'), icon: LayoutGrid, color: 'from-primary-500 to-secondary-500' },
//     { id: 'audio', label: t('search.audio', 'Audio'), icon: Headphones, color: 'from-amber-500 to-orange-500' },
//     { id: 'authors', label: t('search.authors', 'Authors'), icon: Users, color: 'from-blue-500 to-cyan-500' },
//     { id: 'poetry', label: t('search.poetry', 'Poetry'), icon: BookOpen, color: 'from-emerald-500 to-teal-500' },
//     { id: 'books', label: t('search.books', 'Books'), icon: BookMarked, color: 'from-purple-500 to-pink-500' },
//     { id: 'videos', label: t('search.videos', 'Videos'), icon: Video, color: 'from-red-500 to-rose-500' },
//   ], [t])

//   // 🔴 FIX: Make audio categories dynamic with useMemo
//   const audioCategories = useMemo(() => [
//     { path: '/audio/type/nauha', label: t('audio.nauha', 'Nauha'), icon: '😢', count: '245+' },
//     { path: '/audio/type/marsiya', label: t('audio.marsiya', 'Marsiya'), icon: '💔', count: '189+' },
//     { path: '/audio/type/majlis', label: t('audio.majlis', 'Majlis'), icon: '🕌', count: '156+' },
//     { path: '/audio/type/soz', label: t('audio.soz', 'Soz'), icon: '🔥', count: '98+' },
//     { path: '/audio/type/ghazal', label: t('audio.ghazal', 'Ghazal'), icon: '🎵', count: '1.2k+' },
//     { path: '/audio/type/nazm', label: t('audio.nazm', 'Nazm'), icon: '📝', count: '876+' },
//     { path: '/audio/type/podcast', label: t('audio.podcast', 'Podcast'), icon: '🎙️', count: '432+' },
//     { path: '/audio/type/mushaira', label: t('audio.mushaira', 'Mushaira'), icon: '🎤', count: '234+' },
//   ], [t])

//   // 🔴 FIX: Make occasion categories dynamic with useMemo
...
//               id: item._id,
//               title: item.title,
//               description: item.description || `${item.type} ${t('search.audio', 'audio')}`,
//               url: `/audio/${item.slug}`,
//               type: 'audio',
//               icon: <Headphones className="h-4 w-4" />
//             }))
...
//               id: item._id,
//               title: item.name,
//               description: item.bio?.substring(0, 100) || `${item.era || t('search.poet', 'Poet')} - ${item.genres?.join(', ') || t('search.author', 'Author')}`,
//               url: `/author/${item.slug}`,
//               type: 'author',
//               icon: <Users className="h-4 w-4" />
//             }))
...
//               id: item._id,
//               title: item.title,
//               description: item.description || t('search.poemRecitation', 'Poem recitation'),
//               url: `/poem/${item.slug || item._id}`,
//               type: 'poetry',
//               icon: <BookOpen className="h-4 w-4" />
//             }))
...
//               id: item._id,
//               title: item.title,
//               description: item.description || t('search.audiobook', 'Audiobook'),
//               url: `/book/${item.slug || item._id}`,
//               type: 'book',
//               icon: <BookMarked className="h-4 w-4" />
//             }))
...
//               id: item._id,
//               title: item.title,
//               description: item.description || `${item.type} ${t('search.content', 'content')}`,
//               url: `/video/${item.slug || item._id}`,
//               type: 'video',
//               icon: <Video className="h-4 w-4" />
//             }))
...
//   const startVoiceSearch = () => {
//     if (!voiceSupported) {
//       toast.error(t('search.voiceNotSupported', 'Voice search is not supported in your browser'))
//       return
//     }
//     if (recognitionRef.current) {
//       recognitionRef.current.start()
...
//           </p>
//         </div>
//         <div className="text-xs text-gray-400 capitalize">{t(`search.${result.type}`, result.type)}</div>
//       </div>
//     )
//   }
...
//                               <div className="flex items-center gap-2">
//                                 <Headphones className="h-4 w-4 text-primary-600" />
//                                 <h3 className="font-bold text-gray-900 dark:text-white">{t('audio.audioLibrary', 'Audio Library')}</h3>
//                               </div>
//                               <p className="text-xs text-gray-500">{t('audio.discoverRecitations', 'Discover soulful recitations')}</p>
//                             </div>
//                           </div>
//                         </div>
...
//                           {/* Categories Column */}
//                           <div>
//                             <p className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">{t('audio.categories', 'Categories')}</p>
//                             <div className="space-y-1">
//                               {audioCategories.map((category) => (
//                                 <Link
//                                   key={category.path}
...
//                           {/* Occasions Column */}
//                           <div>
//                             <p className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">{t('audio.occasions', 'Occasions')}</p>
//                             <div className="space-y-1">
//                               {occasionCategories.map((occasion) => (
//                                 <Link
//                                   key={occasion.path}
...
//                 <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-900 rounded-xl shadow-lg border py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
//                   <Link to="/dashboard" className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-primary-50">{t('nav.dashboard', 'Dashboard')}</Link>
//                   <Link to="/dashboard/favorites" className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-primary-50">{t('nav.favorites', 'Favorites')}</Link>
//                   <Link to="/dashboard/history" className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-primary-50">{t('nav.history', 'History')}</Link>
//                   {getUserRole() === 'admin' && (
//                     <Link to="/admin" className="flex items-center gap-2 px-3 py-2 text-sm text-purple-600 hover:bg-purple-50">{t('nav.admin', 'Admin')}</Link>
//                   )}
//                   <div className="border-t my-1"></div>
//                   <button onClick={logout} className="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50">{t('nav.signOut', 'Sign Out')}</button>
//                 </div>
//               </div>
//             ) : (
//               <div className="flex items-center gap-1">
//                 <Link to="/login" className="hidden sm:flex px-3 py-2 text-sm font-semibold text-gray-700 hover:text-primary-600">{t('nav.signIn', 'Sign In')}</Link>
//                 <Link to="/register" className="px-4 py-2 text-sm font-bold text-white rounded-lg hover:shadow-md transition" style={getGradientStyle()}>
...
//                 <div className="border-t my-2"></div>
//                 <Link to="/login" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-gray-700 hover:bg-gray-50 rounded-lg">
//                   <LogIn className="h-5 w-5" /> {t('nav.signIn', 'Sign In')}
//                 </Link>
//                 <Link to="/register" onClick={() => setIsMenuOpen(false)} className="flex items-center justify-center px-3 py-2.5 text-white rounded-lg mt-2" style={getGradientStyle()}>
...
//   // Dynamic navLinks with translations
//   const navLinks = useMemo(() => [
//     { path: '/', label: t('nav.home', 'Home'), icon: Compass },
//     { path: '/explore', label: t('nav.explore', 'Explore'), icon: Sparkles },
//     { path: '/poetry', label: t('nav.poetry', 'Poetry'), icon: BookOpen },
...
//   // Audio categories with translations
//   const audioCategories = useMemo(() => [
...
//                                   <span className="text-base">{category.icon}</span>
//                                   <span className="flex-1 font-medium group-hover:text-amber-600">{category.label}</span>
//                                   <span className="text-[9px] text-gray-400">{category.count}</span>
//                                 </Link>
//                               ))}
//                             </div>
//                           </div>
...
//                                   to={occasion.path}
//                                   onClick={() => setIsAudioDropdownOpen(false)}
//                                   className="flex items-center justify-between px-2 py-1.5 rounded-lg text-xs text-gray-700 hover:bg-primary-50 transition-all group"
//                                 >
//                                   <div className="flex items-center gap-2">
//                                     <span className="text-base">{occasion.icon}</span>
//                                     <span className="font-medium group-hover:text-primary-600">{occasion.label}</span>
...
//               {isSearchOpen && (
//                 <div className="absolute right-0 mt-2 w-[90vw] md:w-[500px] bg-white rounded-xl shadow-2xl border border-gray-200/50 overflow-hidden z-50 animate-fadeInUp">
//                   <div className="p-3 border-b border-gray-200/50">
//                     <div className="relative">
//                       <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
//                       <input
...
// const SUPPORTED_LANGUAGES = [
//   { code: 'en', name: 'English', native: 'English', flag: '🇬🇧', dir: 'ltr' },
//   { code: 'ur', name: 'Urdu', native: 'اردو', flag: 'UR', dir: 'rtl' },
//   { code: 'hi', name: 'Hindi', native: 'हिन्दी', flag: '🇮🇳', dir: 'ltr' }
// ]
...
//       }
//       faviconLink.href = siteSettings.siteFavicon
//       console.log('🔄 Favicon updated:', siteSettings.siteFavicon)
//     }
//   }, [siteSettings.siteFavicon, faviconError])

//   // Update logo on error fallback
//   useEffect(() => {
//     if (siteSettings.siteLogo) {
//       console.log('🔄 Logo URL:', siteSettings.siteLogo)
//     }
...
//     { path: '/blog', label: t('common.blog', 'Blog'), icon: Newspaper }, // ✅ Blog added
//   ]
...
//   // Dynamic settings state - Default values
//   const [siteSettings, setSiteSettings] = useState({
//     siteName: 'ZauqApp',
//     siteDescription: 'AI Powered Urdu Literary Ecosystem',
//     siteLogo: '',
...
//           }
//           setSiteSettings(newSettings)
//           console.log('✅ Site settings applied:', newSettings)
//           console.log('✅ Logo URL from settings:', newSettings.siteLogo)
...
//   // ✅ FIXED LOGO RENDER FUNCTION - Proper error handling
//   const renderLogo = () => {
//     // If logo URL exists and no error loading it
...
//     // Fallback: Show icon-based logo
//     console.log('🎨 Rendering fallback icon logo (no logo URL or error)')
//     const LogoIcon = BookOpen
//     return (
//       <div className="relative group">
//         <div className="absolute -inset-1 bg-gradient-to-r from-amber-500 to-rose-500 rounded-xl blur opacity-0 group-hover:opacity-60 transition duration-300"></div>
...
//           {/* Logo - Dynamic with fixed rendering */}
//           <Link 
//             to="/" 
//             className="flex items-center space-x-3 group cursor-pointer"
//           >
