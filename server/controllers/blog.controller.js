...
// // incorrect server/controllers/blog.controller.js
// import Blog from '../models/Blog.js';
// import User from '../models/User.js';
// import { successResponse, errorResponse, paginatedResponse } from '../utils/response.js';
// import { getPagination } from '../utils/pagination.js';
...
//   try {
//     const { page, limit, skip } = getPagination(req);
//     const { category, tag, search, featured, author, status = 'published' } = req.query;
...
//     const blogs = await Blog.find(filter)
//       .populate('author', 'name nameUrdu nameHindi avatar slug bio')
//       .sort({ publishedAt: -1, createdAt: -1 })
//       .skip(skip)
//       .limit(limit)
//       .select('title titleUrdu titleHindi slug excerpt featuredImage createdAt publishedAt views likes commentsCount category tags');
...
//       status: 'published'
//     })
//       .populate('author', 'name nameUrdu nameHindi avatar slug')
//       .sort({ publishedAt: -1, views: -1 })
//       .skip(skip)
//       .limit(parseInt(limit))
//       .select('title titleUrdu titleHindi slug excerpt featuredImage createdAt publishedAt views likes commentsCount');
...
//           latestBlog: { $arrayElemAt: ['$blogs', 0] }
//         }
//       },
//       {
...
//           latestBlogSlug: '$latestBlog.slug'
//         }
//       }
//     ]);
...
//       .sort({ views: -1 })
//       .limit(parseInt(limit))
//       .select('title slug excerpt featuredImage views likes commentsCount publishedAt');
...
//       .populate('author', 'name nameUrdu nameHindi avatar slug bio bioUrdu bioHindi');
...
//     if (req.user) {
//       isLiked = blog.likedBy && blog.likedBy.includes(req.user.id);
//       isBookmarked = blog.bookmarkedBy && blog.bookmarkedBy.includes(req.user.id);
//     }
...
//     const blog = await Blog.findById(id)
//       .populate('author', 'name nameUrdu nameHindi avatar email');
...
//       .sort({ publishedAt: -1 })
//       .limit(parseInt(limit))
//       .select('title titleUrdu titleHindi slug excerpt featuredImage publishedAt views likes commentsCount');
...
//       .sort({ publishedAt: -1, createdAt: -1 })
//       .limit(parseInt(limit))
//       .select('title slug featuredImage excerpt publishedAt views likes commentsCount');
...
//     const totalBookmarks = await Blog.aggregate([
//       { $match: { status: 'published' } },
//       { $group: { _id: null, total: { $sum: '$bookmarks' } } }
//     ]);
...
//     const likeIndex = blog.likedBy ? blog.likedBy.indexOf(userId) : -1;
...
//     const bookmarkIndex = blog.bookmarkedBy ? blog.bookmarkedBy.indexOf(userId) : -1;
    
//     if (bookmarkIndex !== -1) {
//       blog.bookmarkedBy.splice(bookmarkIndex, 1);
//       blog.bookmarks = Math.max(0, blog.bookmarks - 1);
//       await blog.save();
//       return successResponse(res, { bookmarked: false, bookmarks: blog.bookmarks });
//     }
...
//     successResponse(res, { bookmarked: true, bookmarks: blog.bookmarks });
//   } catch (error) {
...
//   try {
//     const { 
//       title, titleUrdu, titleHindi, excerpt, content, category, tags, 
//       featuredImage, gallery, videoUrl, pdfUrl, 
//       isPublished, isFeatured, seoTitle, seoDescription, metaKeywords 
//     } = req.body;
...
//     const populatedBlog = await Blog.findById(blog._id)
//       .populate('author', 'name nameUrdu nameHindi avatar');
...
//     if (blogs.length > 100) {
//       return errorResponse(res, 'Maximum 100 blogs per upload', 400);
//     }
...
//     successResponse(res, { isPublished: blog.isPublished, status: blog.status }, `Blog ${blog.isPublished ? 'published' : 'unpublished'} successfully`);
//   } catch (error) {
//     console.error('Error in togglePublish:', error);
//     next(error);
//   }
...
//     successResponse(res, { uploaded, failed, errors }, `${uploaded} blogs uploaded successfully`);
//   } catch (error) {
//     console.error('Error in bulkUploadBlogs:', error);
//     next(error);
//   }
