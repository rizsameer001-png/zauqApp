...
export const getVideos = async (req, res, next) => {
  try {
    ...
    const videos = await query.lean().exec(); // <-- ADD .lean() for better performance
    const total = await Video.countDocuments(filters);
    
    // Ensure sourceType is always present in response
    const videosWithSourceType = videos.map(v => ({
      ...v,
      sourceType: v.sourceType || (v.videoUrl?.includes('youtube') || v.videoUrl?.includes('youtu.be') ? 'youtube' : 'upload')
    }));
    
    paginatedResponse(res, videosWithSourceType, { page, limit, total });
  } catch (error) {
    console.error('Error in getVideos:', error);
    next(error);
  }
...
export const getVideoBySlug = async (req, res, next) => {
  try {
    ...
    // Ensure sourceType is always present
    if (!videoObj.sourceType) {
      videoObj.sourceType = (videoObj.videoUrl?.includes('youtube') || videoObj.videoUrl?.includes('youtu.be')) ? 'youtube' : 'upload';
    }

    successResponse(res, { ...videoObj, userInteraction });
  } catch (error) {
    console.error('Error in getVideoBySlug:', error);
    next(error);
  }
...
export const getVideoStream = async (req, res, next) => {
  try {
    ...
    // Determine sourceType if not set
    const determinedSourceType = video.sourceType || 
      ((video.videoUrl?.includes('youtube.com') || video.videoUrl?.includes('youtu.be')) ? 'youtube' : 'upload');
