...
// // ============================================
// // GET AUTHORS - WITH ENHANCED SEARCH SUPPORT
// // ============================================
// export const getAuthors = async (req, res, next) => {
//   try {
//     const { page, limit, skip } = getPagination(req);
//     const sort = getSort(req, 'name');
//     let filters = getFilters(req, ['era', 'category', 'language']);
    
//     // Handle search query
//     const searchQuery = req.query.search;
//     console.log('📡 getAuthors called with:', { 
//       page, 
//       limit, 
//       search: searchQuery, 
//       filters, 
...
//         { bio: searchRegex },
//         { bioUrdu: searchRegex },
//         { bioHindi: searchRegex },
//         { genres: { $in: [searchRegex] } },
//         { category: searchRegex },
//         { era: searchRegex }
//       ];
      
//       // If search term is short (2-3 characters), only search name fields for better performance
//       if (searchTerm.length <= 3) {
//         filters.$or = searchConditions.slice(0, 3); // Only name fields
//         console.log('🔍 Short search term, searching only name fields');
//       } else {
//         filters.$or = searchConditions; // All fields
//         console.log('🔍 Long search term, searching all fields');
//       }
//     }
...
//     console.log(`✅ Found ${authors.length} authors out of ${total} total`);
...
//     const author = await Author.findOne({ slug })
//       .populate('relatedAuthors', 'name slug avatar bio era');
...
//     let isFollowing = false;
//     if (req.user) {
//       isFollowing = author.followers ? author.followers.includes(req.user.id) : false;
//     }
...
//       ...author.toObject(),
//       isFollowing,
//       formattedBirthDate: author.birthDate ? new Date(author.birthDate).toLocaleDateString() : null,
//       formattedDeathDate: author.deathDate ? new Date(author.deathDate).toLocaleDateString() : null,
//       years: author.years
//     };
...
//       return errorResponse(res, 'Bio is required', 400);
//     }
...
//     // Optional: Update poems to remove author reference or mark as orphaned
//     await Poem.updateMany({ author: id }, { $unset: { author: "" } });
...
//     author.timeline.sort((a, b) => a.year - b.year);
//     await author.save();
...
//     author.gallery = author.gallery || [];
//     author.gallery.push({ url, caption: caption || '', uploadedAt: new Date() });
//     await author.save();
...
//     successResponse(res, { followed: false, followersCount: author.stats.followers || 0 }, 'Author unfollowed');
//   } catch (error) {
//     console.error('Error in unfollowAuthor:', error);
//     next(error);
//   }
...
// // ============================================
// // SEARCH AUTHORS (Dedicated search endpoint)
// // ============================================
...
//           totalPages: 0
//         }
//       }, 'No search query provided (minimum 2 characters)');
//     }
...
//     const authors = await Author.find(searchConditions)
//       .sort({ name: 1 })
//       .skip(skip)
//       .limit(parseInt(limit));
...
//   try {
//     const { slug } = req.params;
//     const author = await Author.findOne({ slug }).populate('relatedAuthors', 'name slug avatar bio era stats');
...
// ============================================
// GET AUTHORS - WITH ENHANCED SEARCH AND LETTER FILTERING
// ============================================
export const getAuthors = async (req, res, next) => {
  try {
    const { page, limit, skip } = getPagination(req);
    ...
    // Handle letter filter (alphabetical index)
    const letterFilter = req.query.letter;
    ...
    // Add letter filter if provided and no search query
    if (letterFilter && letterFilter.trim() && !searchQuery) {
      let letter = letterFilter.trim().toUpperCase();
      
      // Handle special case for V/W (authors starting with V or W)
      if (letter === 'V/W') {
        filters.name = { $regex: '^[VWvw]', $options: 'i' };
        console.log(`🔍 Filtering by letter: V or W`);
      } 
      // Handle special case for single letters
      else if (letter.length === 1) {
        filters.name = { $regex: `^${letter}`, $options: 'i' };
        console.log(`🔍 Filtering by letter: ${letter}`);
      }
      // Handle other cases
      else {
        filters.name = { $regex: `^${letter.charAt(0)}`, $options: 'i' };
        console.log(`🔍 Filtering by letter: ${letter.charAt(0)}`);
      }
    }
    
    // Add search filter if provided (overrides letter filter)
    if (searchQuery && searchQuery.trim()) {
      const searchTerm = searchQuery.trim();
      const searchRegex = new RegExp(searchTerm, 'i');
      ...
      // Remove letter filter when searching (search overrides letter filter)
      if (letterFilter) {
        delete filters.name;
        console.log('🔍 Search overrides letter filter');
      }
    }
    ...
    // Build query
    let query = Author.find(filters);
    
    // Apply sorting
    query = query.sort(sort);
    
    // Apply pagination
    const authors = await query.skip(skip).limit(limit);
    const total = await Author.countDocuments(filters);
...
export const deleteAuthor = async (req, res, next) => {
  try {
    ...
    // Update poems to remove author reference
    await Poem.updateMany({ author: id }, { $unset: { author: "" } });
